...
void ReadY()
{
	...
	//for (i=0;i<len;i++)
	{
		...
		*(U8 *)(AD1+8)=asicc(a5);	
		*(U8 *)(AD1+9)=0x20;        //SPACE
		*(U8 *)(AD1+10)=asicc((ptcom->register_length * 8)/10);		//长度，8个位
		*(U8 *)(AD1+11)=asicc((ptcom->register_length * 8)%10);		//长度，8个位
		*(U8 *)(AD1+12)=0x0d;   	//	
			
		ptcom->send_length[0]=13;		//发送长度
		ptcom->send_staradd[0]=0;			//发送数据存储地址
		ptcom->send_add[0]=ptcom->address;				//读的是这个地址的数据	
		ptcom->send_data_length[0]=1;	//不是最后一次都是1个字节
					
		ptcom->return_length[0]=ptcom->register_length * 16 + 1;				//返回数据长度
		ptcom->return_start[0]=0;				//返回数据有效开始
		ptcom->return_length_available[0]=ptcom->register_length * 16 - 1;	//返回有效数据长度								
	}	
...
void ReadM()
{
	...
	len=ptcom->register_length;				// 开始读取长度
		...
		*(U8 *)(AD1+12)=asicc((ptcom->register_length * 8)/10);		//长度，8个位
		*(U8 *)(AD1+13)=asicc((ptcom->register_length * 8)%10);		//长度，8个位
		*(U8 *)(AD1+14)=0x0d;   	//	
			
		ptcom->send_length[0]=15;		//发送长度
		ptcom->send_staradd[0]=0;			//发送数据存储地址
		ptcom->send_add[0]=ptcom->address;				//读的是这个地址的数据	
		ptcom->send_data_length[0]=1;	//不是最后一次都是1个字节
...
void ReadL()
{
	...
	//for (i=0;i<len;i++)
	{
		...
		*(U8 *)(AD1+14)=0x0d;   	//	
		...
		ptcom->send_staradd[i]=0;			//发送数据存储地址
		ptcom->send_add[i]=ptcom->address;				//读的是这个地址的数据	
		ptcom->send_data_length[i]=1;	//不是最后一次都是1个字节
					
		ptcom->return_length[i]=ptcom->register_length * 16 + 1;				//返回数据长度
		ptcom->return_start[i]=0;				//返回数据有效开始
		ptcom->return_length_available[i]=ptcom->register_length * 16 -  1;	//返回有效数据长度								
	}	
	//ptcom->register_length=1;
	ptcom->send_times=1;					//发送次数
	ptcom->Current_Times=0;					//当前发送次数	
...
void compxy(void)				              //处理成标准存储格式,重新排列
{
	...
	if (ptcom->Simens_Count==1)	//读的是位,每次都是读一个字节
	{
		...
		ptcom->return_length_available[ptcom->Current_Times-1]=j/8;	
