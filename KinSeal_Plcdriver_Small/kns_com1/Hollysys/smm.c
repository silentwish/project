...
void Read_Analog()				//读模拟量
{
	...
	b=ptcom->address/2;			      //在主程序已经转换到该段的开始地址
	plcadd=ptcom->plc_address;	      //PLC站地址
	length=ptcom->register_length;
	
	b=b+add;					      //地址 加上偏移地址,在上位软件中已经做处理把地址缩小一倍
	a1=(b>>8)&0xff;
	a2=b&0xff;
...
void Read_Recipe()								//读取配方
{
	...
	if(datalength%28==0)
	{
		SendTimes=datalength/28;
		LastTimeWord=28;                //固定长度32	
	}
	if(datalength%28!=0)
	{
		SendTimes=datalength/28+1;      //发送的次数
		LastTimeWord=datalength%28;     //最后一次发送的长度	
	}
	
	for (i=0;i<SendTimes;i++)
	{
		ps=8*i;                         //每次发8个长度
		b=(p_start+3000+i*32);		//加上偏移地址,在上位软件中已经做处理把地址缩小一倍
...
void Write_Recipe()								//写配方到PLC
{
	...
	if(datalength%28==0)           //每次最多能发送32个D，如果是长度是32个D的倍数
	{
		SendTimes=datalength/28;   //发送的次数
		LastTimeWord=28;           //最后一次发送的长度	
	}
	if(datalength%28!=0)           //如果不是长度是32个D的倍数
	{
		SendTimes=datalength/28+1; //发送的次数
		LastTimeWord=datalength%28;//最后一次发送的长度	
	}	
	...
	for (i=0;i<SendTimes;i++)
	{
		b=staradd+3000+i*28;           //起始地址，要+偏移地址,在上位软件中已经做处理把地址缩小一倍
		a1=(b>>8)&0xff;
		a2=b&0xff;
		...
		*(U8 *)(AD1+6+ps*i)=length*2;
		...
		for(j=0;j<length;j++)
		{
			*(U8 *)(AD1+7+ps*i+j*2)=*(U8 *)(PE+9+i*56+j*2+1);	  //取数据
			*(U8 *)(AD1+7+ps*i+j*2+1)=*(U8 *)(PE+9+i*56+j*2);			
		}
