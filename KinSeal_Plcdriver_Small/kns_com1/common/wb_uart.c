...
VOID sysUartISR()
{
 ...
 VOID sysSetBaudRate(UINT32 uBaudRate)
{
	UINT32 _mBaudValue;
...
void tx_enable()
{
	...
	outpb((REG_UART_IER+u32UartPort), inp32(REG_UART_IER+u32UartPort) & (~RDA_IEN));
	outpb((REG_UART_IER+u32UartPort), inp32(REG_UART_IER+u32UartPort) & (~RTO_IEN)& (~Time_out_EN));
	outp32(REG_UART_IER+u32UartPort, inp32(REG_UART_IER+u32UartPort) |THRE_IEN);
}
void disable_uart(void)
{
 
 //   while (_sys_uUartTxHead != _sys_uUartTxTail)
  //      OSTimeDly(1); 
    outpb((REG_UART_IER+u32UartPort), inp32(REG_UART_IER+u32UartPort) & (~THRE_IEN));
...
*/
		if (!(inpw(REG_UART_IER+0x100) & 0x02))
			outpw(REG_UART_IER+0x100, 0x02);
	}
	else
	{
		...
		outpb(REG_UART_THR+0x100, ucCh);

		if (ucCh == '\n')
		{
			/* Wait until the transmitter buffer is empty */
		    	while (!(inpw(REG_UART_FSR+0x100) & 0x400000));
			outpb(REG_UART_THR+0, '\r');
		}
	}
}
...
VOID sysOutString(INT8 *string,INT count)
{
	while (count--)
	{
		_PutChar_f(*string);
...
//	tx_disable();
//	return 1;
	if( (inp32(REG_UART_FSR+u32UartPort) & Tx_Empty) ) 	
//	if (_sys_uUartTxHead == _sys_uUartTxTail) 
	{
		outpb((REG_UART_IER+u32UartPort), inp32(REG_UART_IER+u32UartPort) & (~THRE_IEN));
		sysUartEnableInt(UART_INT_RDA);
